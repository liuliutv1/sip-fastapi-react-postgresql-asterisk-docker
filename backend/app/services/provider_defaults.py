...
CARRIER_DESCRIPTION = "Whitelisted SIP carrier endpoint 218.245.102.33:6876, caller ID 02032730801, Aliyun ECS 8.163.96.127"
...
def ensure_carrier_sip_trunk(db: Session) -> None:
    ...
    trunk.host = settings.sip_vendor_ip or CARRIER_HOST
    trunk.port = settings.sip_vendor_port or CARRIER_PORT
    trunk.transport = "udp"
    trunk.from_user = CARRIER_CALLER_ID
    trunk.from_domain = settings.sip_vendor_ip or CARRIER_HOST
    trunk.outbound_proxy = f"sip:{trunk.host}:{trunk.port}"
    ...
    whitelist = (
        ...
        .filter(
            ...
            models.SipPeerWhitelist.peer_cidr == f"{trunk.host}/32",
        )
        .first()
    )
    if whitelist is None:
        whitelist = models.SipPeerWhitelist(
            ...
            peer_cidr=f"{trunk.host}/32",
        )
        db.add(whitelist)

    whitelist.name = f"Carrier SBC {trunk.host}"
    whitelist.description = f"Carrier SIP peer IP provided for whitelist access; Aliyun ECS {settings.aliyun_public_ip}"
