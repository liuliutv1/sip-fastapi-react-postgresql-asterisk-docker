...
def active_call_count_for_trunk(db: Session, trunk_id: int) -> int:
    count = (
...
def trunk_has_capacity(db: Session, trunk: models.SipTrunk) -> bool:
    max_channels = max(int(trunk.max_channels or 1), 1)
    return active_call_count_for_trunk(db, trunk.id) < max_channels
...
def candidate_trunks(
    ...
    *,
    ...
    return [
        ...
        if trunk.id not in exclude_ids and trunk_has_capacity(db, trunk)
    ]
